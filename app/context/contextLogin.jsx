...
export const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        // Récupérer les favoris, l'utilisateur, et les réservations stockés lors du montage du composant
        const loadFromStorage = async () => {
            try {
                const storedFavorites = await AsyncStorage.getItem("favorites");
                const storedUser = await AsyncStorage.getItem("user");
    ...
    const saveReservationsToStorage = async (reservations) => {
        try {
            ...
            console.log("Error saving reservations to storage", error);
        }
    };
